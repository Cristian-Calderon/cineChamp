...
export default function TodasLasCalificaciones() {
    ...
    async function fetchCalificaciones() {
      try {
        const response = await fetch(
          `http://localhost:3001/api/contenido/usuarios/${id_usuario}/calificaciones`,
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error(
            data?.error || "Error al cargar calificaciones"
          );
        }

        // Comprobamos que la API devuelve un array
        if (!Array.isArray(data)) {
          console.error(
            "❌ La API no devolvió un array de calificaciones:",
            data
          );
  ...
  const totalPaginas = Math.ceil(
    calificaciones.length / ITEMS_POR_PAGINA
  );
              ...
              {calificacionesPaginadas.map((item) => (
                ...
                >
                  ...
                  <div className="mt-3 text-center">
                    ...
                    {item.comentario && (
                      <p className="text-sm italic text-gray-600 mt-1 line-clamp-3">
                        “{item.comentario}”
                      </p>
                    )}
                  </div>
